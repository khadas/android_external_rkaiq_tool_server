...
/* FEC: 4个mesh 内存申请 */
void mallocFecMesh(int meshSize, unsigned short** pMeshXI, unsigned char** pMeshXF, unsigned short** pMeshYI,
...
*/
bool genFECMeshNLevel(FecParams& fecParams, CameraCoeff& camCoeff, int level, unsigned short* pMeshXI,
                      unsigned char* pMeshXF, unsigned short* pMeshYI, unsigned char* pMeshYF);
...
bool cropFecMesh(FecParams& fecParams, RoiParams& roiParams, int level, unsigned short* pCropMeshXI,
                 unsigned char* pCropMeshXF, unsigned short* pCropMeshYI, unsigned char* pCropMeshYF);

/* 根据输出图像的ROI参数，对LDCH mesh进行裁剪, 得到最终需求的尺寸 */
bool cropLdchMesh(LdchParams& ldchParams, RoiParams& roiParams, int level, unsigned short* pMeshX,
                  unsigned short* pRoiMeshX);

/* FEC: 初始化, 用于8k ---> 2个4k */
void genFecMeshInit8kTo4k(int srcW, int srcH, int dstW, int dstH, int margin, CameraCoeff& camCoeff,
                          CameraCoeff& camCoeff_left, CameraCoeff& camCoeff_right, FecParams& fecParams,
                          FecParams& fecParams_left, FecParams& fecParams_right);

/* LDCH: 8k ---> 2个4k */
void genLdchMeshInit8kTo4k(int srcW, int srcH, int dstW, int dstH, int margin, CameraCoeff& camCoeff,
                           CameraCoeff& camCoeff_left, CameraCoeff& camCoeff_right, LdchParams& ldchParams,
                           LdchParams& ldchParams_left, LdchParams& ldchParams_right);

 ...
 * v2.0.1
 *  - date: 2020-09-18
 *  - FEC&LDCH: due to a bug in polynomial fitting, last version will returns error when the I/O resolutions are
different, it has been solved in this version
    - FEC: the fixed-point number is changed from 7 to 8

 * v2.0.2
 *  - date: 2020-09-23
 *  - FEC&LDCH: adjust some code to facilitate branch prediction logic and reduce unnecessary memory references to
improve efficiency
 ...
 *  - FEC: consider the case when the width and height of the output image are different from the input,
           the naming rules of mesh table are specified as(using "meshxi, level=0, input=3840x2160, output=1920x1080,
both x and y direction are corrected" as an example): level000_1920x1080from3840x2160_both_correct_meshxi.bin
 ...
 *  - LDCH: add an option: saveMaxFovX
            ...
            when saveMaxFovX=1, users choose to save max FOV in X direction and tolerate slight compression distortion
in middle part.
 ...
 *  - genCropScaleMeshNLevel:
           add a new function interface: LDCH mesh(for x direction) + FEC mesh(for y direction)
           1) CameraCoeff of original resolution, e.g. 3840*2160 --> LDCH mesh --> LDCH
           2) CameraCoeff of original resolution, e.g. 3840*2160 --> crop at any position(pW,pH) to get any size, e.g.
1600*900 --> scale to 1920*1080 FEC mesh + 1920*1080 image data --> FEC
 ...
 * v4.0.4
 *  - date: 2021-07-05
 *  - LDCH: modify the middle row of mesh(minus 6 pixels) when saveMaxFovX == 1 for old LDCH bug(avoid the last pixel in
the row map to the interval of [last-5, last-3]). valid flag: need to set isLdchOld = 1.

 * v4.0.5
 *  - date: 2021-08-12
 *  - LDCH: last version didnot completely solve this bug: the middle row of mesh(minus 6 pixels) when saveMaxFovX == 1
for old LDCH bug(avoid the last pixel in the row map to the interval of [last-5, last-3]). valid flag: need to set
isLdchOld = 1.

 * v4.0.6
 *  - date: 2021-08-14
 *  - LDCH: consider these case:
            1.rhoMaxW < rohMaxH for old LDCH bug(avoid the last pixel in the row map to the interval of [last-5,
last-3]). 2.identity mesh between penultimate and last rows may cause old LDCH bug when interpolation. valid flag: need
to set isLdchOld = 1.

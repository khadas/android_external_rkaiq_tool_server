...
namespace RkCam
{
    ...
    static const char* Cam3aResultType2Str[RESULT_TYPE_MAX_PARAM] = {[RESULT_TYPE_EXPOSURE_PARAM] = "EXPOSURE",
    ...
    class RkAiqFullParams : public XCam::BufferData
    {
      public:
        explicit RkAiqFullParams()
            : mExposureParams(NULL), mFocusParams(NULL), mIrisParams(NULL), mCpslParams(NULL)

              ,
              mAecParams(NULL), mHistParams(NULL), mAwbParams(NULL), mAwbGainParams(NULL), mAfParams(NULL),
              mDpccParams(NULL), mMergeParams(NULL), mTmoParams(NULL), mCcmParams(NULL), mLscParams(NULL),
              mBlcParams(NULL), mRawnrParams(NULL), mGicParams(NULL), mDebayerParams(NULL), mLdchParams(NULL),
              mLut3dParams(NULL), mDehazeParams(NULL), mAgammaParams(NULL), mWdrParams(NULL), mCsmParams(NULL),
              mCgcParams(NULL), mConv422Params(NULL), mYuvconvParams(NULL), mGainParams(NULL), mCpParams(NULL),
              mIeParams(NULL), mMotionParams(NULL), mMdParams(NULL)

              ,
              mTnrParams(NULL), mYnrParams(NULL), mUvnrParams(NULL), mSharpenParams(NULL), mEdgefltParams(NULL),
              mFecParams(NULL), mOrbParams(NULL)
              // TODO: change full params to list
              // V21 differential modules
              ,
              mAwbV21Params(NULL), mDrcParams(NULL), mBlcV21Params(NULL), mBaynrV21Params(NULL)
              // , mBa3dParams(NULL)
              ,
              mYnrV21Params(NULL), mCnrV21Params(NULL), mSharpenV21Params(NULL), mAwbV3xParams(NULL),
              mAfV3xParams(NULL), mBaynrV3xParams(NULL), mYnrV3xParams(NULL), mCnrV3xParams(NULL),
              mSharpenV3xParams(NULL), mCacV3xParams(NULL), mGainV3xParams(NULL), mTnrV3xParams(NULL){};
        ~RkAiqFullParams()
        {
            reset();
        };
    ...
    template <class T> struct AlgoRstShared : public XCam::BufferData {
      public:

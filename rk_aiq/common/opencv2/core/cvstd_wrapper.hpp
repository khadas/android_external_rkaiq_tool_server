...
namespace cv
{
    ...
    using Ptr = std::shared_ptr<_Tp>; // In ideal world it should look like this, but we need some compatibility
                                      // workarounds below

    template <typename _Tp, typename... A1> static inline Ptr<_Tp> makePtr(const A1&... a1)
    {
    ...
    #if 0
{
    ...
    namespace sfinae
    {
        template <typename C, typename Ret, typename... Args> struct has_parenthesis_operator {
          private:
            template <typename T>
            static CV_CONSTEXPR std::true_type check(
                typename std::is_same<
                    typename std::decay<decltype(std::declval<T>().operator()(std::declval<Args>()...))>::type,
                    Ret>::type*);
    ...
    template <typename T, typename = void> struct has_custom_delete : public std::false_type {
    };
    ...
    using std::shared_ptr<T>::shared_ptr;  // GCC 5.x can't handle this
        ...
        template <typename Y> inline Ptr(const std::true_type&, Y* ptr) : std::shared_ptr<T>(ptr, DefaultDeleter<Y>())
        {
        }

        // Overload without custom deleter: Ptr<std::string>(...);
        template <typename Y> inline Ptr(const std::false_type&, Y* ptr) : std::shared_ptr<T>(ptr)
        {
        }

        template <typename Y = T> inline Ptr(Y* ptr) : Ptr(has_custom_delete<Y>(), ptr)
        {
        }

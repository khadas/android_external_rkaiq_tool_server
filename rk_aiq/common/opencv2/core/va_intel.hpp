...
namespace cv
{
    namespace va_intel
    {
        ...
        To enable CL-VA interoperability support, configure OpenCV using CMake with WITH_VA_INTEL=ON . Currently VA-API
        is supported on Linux only. You should also install Intel Media Server Studio (MSS) to use this feature. You may

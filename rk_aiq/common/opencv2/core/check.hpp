...
#define CV__DEFINE_CHECK_CONTEXT(id, message, testOp, p1_str, p2_str)                                                  \
    static const cv::detail::CheckContext CV__CHECK_LOCATION_VARNAME(id) = {                                           \
        ...
        CV_EXPORTS void CV_NORETURN check_failed_auto(const Size_<int> v1, const Size_<int> v2,
...
#define CV__CHECK_CUSTOM_TEST(id, type, v, test_expr, v_str, test_expr_str, msg_str)                                   \
    do {                                                                                                               \
        if (!!(test_expr))                                                                                             \
            ;                                                                                                          \
        else {                                                                                                         \
...
#define CV_Check(v, test_expr, msg) CV__CHECK_CUSTOM_TEST(_, auto, v, (test_expr), #v, #test_expr, msg)
    ...
    /// Some complex conditions: CV_Check(src2, src2.empty() || (src2.type() == src1.type() && src2.size() ==
    /// src1.size()), "src2 should have same size/type as src1")

        ...
        namespace logging
        {
            ...
            */
            ...
            namespace internal
            {
                ...
                /** Write log message */
                CV_EXPORTS void writeLogMessageEx(LogLevel logLevel, const char* tag, const char* file, int line,
                                                  const char* func, const char* message);

            } // namespace internal

            struct LogTagAuto : public LogTag {
                inline LogTagAuto(const char* _name, LogLevel _level) : LogTag(_name, _level)
                {
                    registerLogTag(this);
                }
            };
 ...
 *
 * Define CV_LOG_STRIP_LEVEL=CV_LOG_LEVEL_[DEBUG|INFO|WARN|ERROR|FATAL|SILENT] to compile out anything at that and
 * before that logging level
 */
...
#define CV_LOG_WITH_TAG(tag, msgLevel, ...)                                                                            \
    for (;;) {                                                                                                         \
        ...
        cv::utils::logging::internal::writeLogMessageEx(cv_temp_msglevel,                                              \
                                                        (cv_temp_logtagptr ? cv_temp_logtagptr->name : nullptr),       \
                                                        __FILE__, __LINE__, CV_Func, cv_temp_logstream.str().c_str()); \
        break;                                                                                                         \
    }

...
namespace cv
{
    namespace utils
    {
        ...
        namespace
        {
...
#else
            volatile long long curr, total, total_allocs,
                peak; // overflow is possible, CV_XADD operates with 'int' only
#endif
              ...
              public:
                ...
                void onAllocate(size_t sz)
                {
                    ...
                    CV__ALLOCATOR_STATS_LOG(
                        cv::format("allocate: %lld (curr=%lld)", (long long int)sz, (long long int)curr.load()));
    #endif
    ...
    #ifdef CV__ALLOCATOR_STATS_LOG
                    CV__ALLOCATOR_STATS_LOG(
                        cv::format("free: %lld (curr=%lld)", (long long int)sz, (long long int)curr.load()));
    #endif
                    curr -= (long long)sz;
                }
            ...
            uint64_t getNumberOfAllocations() const CV_OVERRIDE
            {
                return (uint64_t)total_allocs;
            }
            uint64_t getPeakUsage() const CV_OVERRIDE
            {
                return (uint64_t)peak;
            }
    ...
    #ifdef CV__ALLOCATOR_STATS_LOG
                CV__ALLOCATOR_STATS_LOG(
                    cv::format("allocate: %lld (curr=%lld)", (long long int)sz, (long long int)curr));
    #endif

...
namespace cv
{
    ...
    class CV_EXPORTS TLSDataContainer
    {
      ...
      public:
        void cleanup(); //!< Release created TLS data container objects. It is similar to release() call, but it keeps
                        //!< TLS container valid.
    ...
    template <typename T> class TLSDataAccumulator : public TLSData<T>
    {
      ...
      public:
        ...
        void gather(std::vector<T*>& data) const
        {
            ...
            }
            {
                ...
                for (typename std::vector<T*>::const_iterator i = dataFromTerminatedThreads.begin();
         ...
         *
         * @return Vector with associated data. Content is preserved (including lifetime of attached data pointers)
         * until next detachData()/cleanupDetachedData()/cleanup()/release() calls
         */
      ...
      protected:
        ...
        {
            for (typename std::vector<T*>::iterator i = dataFromTerminatedThreads.begin();
                 i != dataFromTerminatedThreads.end(); ++i) {
                deleteDataInstance((T*)*i);
            }
            dataFromTerminatedThreads.clear();

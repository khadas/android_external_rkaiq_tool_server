...
namespace cvflann
{
    ...
    namespace anyimpl
    {
        ...
        template <typename T> struct typed_base_any_policy : base_any_policy {
            ...
            virtual const std::type_info& type() CV_OVERRIDE
            {
                return typeid(T);
            }
        };

        template <typename T> struct small_any_policy CV_FINAL : typed_base_any_policy<T> {
            ...
            virtual void* get_value(void** src) CV_OVERRIDE
            {
                return reinterpret_cast<void*>(src);
            }
            virtual const void* get_value(void* const* src) CV_OVERRIDE
            {
        ...
        template <typename T> struct big_any_policy CV_FINAL : typed_base_any_policy<T> {
        ...
        template <typename T> struct choose_policy {
...
#undef SMALL_POLICY
        ...
        template <typename T> typename choose_policy<T>::type SinglePolicy<T>::policy;

        /// This function will return a different policy for each type.
        template <typename T> inline base_any_policy* SinglePolicy<T>::get_policy()
        {
            return &policy;
        }
    ...
    struct any {
      ...
      public:
        ...
        template <typename T>
        any(const T& x) : policy(anyimpl::SinglePolicy<anyimpl::empty_any>::get_policy()), object(NULL)
        {
            assign(x);
        }

        /// Empty constructor.
        any() : policy(anyimpl::SinglePolicy<anyimpl::empty_any>::get_policy()), object(NULL)
        {
        }

        /// Special initializing constructor for string literals.
        any(const char* x) : policy(anyimpl::SinglePolicy<anyimpl::empty_any>::get_policy()), object(NULL)
        {
            assign(x);
        }

        /// Copy constructor.
        any(const any& x) : policy(anyimpl::SinglePolicy<anyimpl::empty_any>::get_policy()), object(NULL)
        {
            assign(x);
        }
        ...
        /// Assignment operator. Template-based version above doesn't work as expected. We need regular assignment
        /// operator here.

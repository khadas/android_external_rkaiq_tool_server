...
namespace cv
{
    ...
    namespace detail
    {
        ...
        template <typename, typename, typename> struct TransHelper;

        template <typename K, typename... Ins, typename Out> struct TransHelper<K, std::tuple<Ins...>, Out> {
            ...
            static GComputation invoke(Callable f, Seq<IIs...>, Seq<OIs...>)
            {
                ...
                return GComputation(cv::GIn(std::get<IIs>(ins)...), cv::GOut(std::get<OIs>(r)...));
            }
    ...
    template <typename, typename> class GTransformImpl;
...
#define GAPI_TRANSFORM(Class, API, Descr)                                                                              \
    ...
    struct Class final : public cv::GTransformImpl<Class, std::function API>, public detail::G_DESCR_HELPER_CLASS(Class)

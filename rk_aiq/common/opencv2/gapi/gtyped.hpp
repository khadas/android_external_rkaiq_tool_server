...
namespace cv
{
    ...
    namespace detail
    {
        ...
        template <> struct ProtoToParam<cv::GMat> {
            using type = cv::Mat;
        };
        template <> struct ProtoToParam<cv::GScalar> {
            using type = cv::Scalar;
        };
        template <typename U> struct ProtoToParam<cv::GArray<U>> {
        ...
        template <> struct ProtoToMeta<cv::GMat> {
            using type = cv::GMatDesc;
        };
        template <> struct ProtoToMeta<cv::GScalar> {
            using type = cv::GScalarDesc;
        };
        template <typename U> struct ProtoToMeta<cv::GArray<U>> {
            using type = cv::GArrayDesc;
        };
        template <typename T> using ProtoToMetaT = typename ProtoToMeta<T>::type;

        // workaround for MSVC 19.0 bug
        template <typename T> auto make_default() -> decltype(T{})
        {
            return {};
        }
    }; // namespace detail
      ...
      public:
        GComputationT(const Gen& generator)
            : m_capture(capture(generator, detail::make_default<Args>()...)),
              m_comp(cv::GProtoInputArgs(std::move(m_capture.second)), cv::GOut(m_capture.first))
        {
        }
    ...
    template <typename... R, typename... Args> class GComputationT<std::tuple<R...>(Args...)>
    {
      ...
      private:
        ...
        template <int... IIs> Captured capture(GProtoArgs&& args, const std::tuple<R...>& rr, detail::Seq<IIs...>)
        {
      ...
      public:
        GComputationT(const Gen& generator)
            : m_capture(capture(generator, detail::make_default<Args>()...)),
              m_comp(cv::GProtoInputArgs(std::move(m_capture.second)), cv::GProtoOutputArgs(std::move(m_capture.first)))
        {
        }

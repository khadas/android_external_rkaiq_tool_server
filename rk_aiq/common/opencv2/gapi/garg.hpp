...
namespace cv
{
    ...
    namespace detail
    {
        template <typename T> using is_garg = std::is_same<GArg, typename std::decay<T>::type>;
    }
    ...
    class GAPI_EXPORTS GArg
    {
      public:
        ...
        explicit GArg(const T& t) : kind(detail::GTypeTraits<T>::kind), value(detail::wrap_gapi_helper<T>::wrap(t))
        {
        }

        template <typename T, typename std::enable_if<!detail::is_garg<T>::value, int>::type = 0>
        explicit GArg(T&& t)
            : kind(detail::GTypeTraits<typename std::decay<T>::type>::kind), value(detail::wrap_gapi_helper<T>::wrap(t))
        {
        }
    ...
    using GRunArg = util::variant<
        ...
        cv::Mat, cv::Scalar, cv::UMat,
#endif // !defined(GAPI_STANDALONE)
        cv::gapi::wip::IStreamSource::Ptr, cv::gapi::own::Mat, cv::gapi::own::Scalar, cv::detail::VectorRef>;
    ...
    namespace gapi
    {
        namespace wip
        {
             ...
             *
             * It only exists to overcome C++ language limitations (where a `using`-defined class can't be
             * forward-declared).
             */
            struct Data : public GRunArg {
                using GRunArg::GRunArg;
                template <typename T> Data& operator=(const T& t)
                {
                    GRunArg::operator=(t);
                    return *this;
                }
                template <typename T> Data& operator=(T&& t)
                {
                    GRunArg::operator=(std::move(t));
                    return *this;
                }
            };
        } // namespace wip
    ...
    using GRunArgP = util::variant<
        ...
        cv::Mat*, cv::Scalar*, cv::UMat*,
#endif // !defined(GAPI_STANDALONE)
        cv::gapi::own::Mat*, cv::gapi::own::Scalar*, cv::detail::VectorRef>;

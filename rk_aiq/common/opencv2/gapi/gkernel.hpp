...
namespace cv
{
    ...
    namespace detail
    {
        ...
        namespace
        {
            ...
            template <> struct Yield<cv::GMat> {
                static inline cv::GMat yield(cv::GCall& call, int i)
                {
                    return call.yield(i);
                }
            };
            template <> struct Yield<cv::GMatP> {
                static inline cv::GMatP yield(cv::GCall& call, int i)
                {
                    return call.yieldP(i);
                }
            };
        ...
        template <> struct MetaType<cv::GMat> {
            using type = GMatDesc;
        };
        template <> struct MetaType<cv::GMatP> {
            using type = GMatDesc;
        };
        template <> struct MetaType<cv::GScalar> {
            using type = GScalarDesc;
        };
        template <typename U> struct MetaType<cv::GArray<U>> {
            using type = GArrayDesc;
        };
        template <typename T> struct MetaType {
            using type = T;
        ...
        template <typename T>
        typename std::enable_if<!is_nongapi_type<T>::value, typename MetaType<T>::type>::type get_in_meta(
            const GMetaArgs& in_meta, const GArgs&, int idx)
        {
            return util::get<typename MetaType<T>::type>(in_meta.at(idx));
        }

        template <typename T>
        typename std::enable_if<is_nongapi_type<T>::value, T>::type get_in_meta(const GMetaArgs&, const GArgs& in_args,
                                                                                int idx)
        {
        ...
        template <typename, typename, typename> struct MetaHelper;
        ...
        struct MetaHelper<K, std::tuple<Ins...>, std::tuple<Outs...>> {
            ...
            {
                return getOutMeta_impl(in_meta, in_args, typename detail::MkSeq<sizeof...(Ins)>::type(),
        ...
        template <typename K, typename... Ins, typename Out> struct MetaHelper<K, std::tuple<Ins...>, Out> {
            template <int... IIs>
            static GMetaArgs getOutMeta_impl(const GMetaArgs& in_meta, const GArgs& in_args, detail::Seq<IIs...>)
            {
                // FIXME: decay?
    ...
    class GKernelTypeM<K, std::function<std::tuple<R...>(Args...)>>
        : public detail::MetaHelper<K, std::tuple<Args...>, std::tuple<R...>>, public detail::NoTag
    {
        template <int... IIs> static std::tuple<R...> yield(cv::GCall& call, detail::Seq<IIs...>)
        {
    ...
    template <typename K, typename R, typename... Args>
    class GKernelType<K, std::function<R(Args...)>> : public detail::MetaHelper<K, std::tuple<Args...>, R>,
                                                      public detail::NoTag
    {
      public:
        using InArgs = std::tuple<Args...>;
...
#define G_TYPED_KERNEL(Class, API, Id)                                                                                 \
    ...
    struct Class final : public cv::GKernelType<Class, std::function API>, public G_ID_HELPER_CLASS(Class)
...
#define G_TYPED_KERNEL_M(Class, API, Id)                                                                               \
    G_ID_HELPER_BODY(Class, Id)                                                                                        \
    struct Class final : public cv::GKernelTypeM<Class, std::function API>, public G_ID_HELPER_CLASS(Class)
// {body} is to be defined by user
    ...
    namespace gapi
    {
        ...
        class GAPI_EXPORTS GKernelPackage
        {
          ...
          protected:
            ...
            typename std::enable_if<(std::is_base_of<detail::KernelTag, KImpl>::value), void>::type includeHelper()
            {
            ...
            typename std::enable_if<(std::is_base_of<detail::TransformTag, TImpl>::value), void>::type includeHelper()
            {
          ...
          public:
             ...
             */
            template <typename KImpl> bool includes() const
            {
                ...
                return kernel_it != m_id_kernels.end() && kernel_it->second.first == KImpl::backend();
            }
             ...
             */
            template <typename KAPI> void remove()
            {
                removeAPI(KAPI::id());
            }
             ...
             * by API type.
             */
            template <typename KAPI> bool includesAPI() const
            {
             ...
             *
             */
            template <typename KAPI> GBackend lookup() const
            {
                return lookup(KAPI::id()).first;
            }

            /// @private
            std::pair<cv::gapi::GBackend, cv::GKernelImpl> lookup(const std::string& id) const;
             ...
             */
            friend GAPI_EXPORTS GKernelPackage combine(const GKernelPackage& lhs, const GKernelPackage& rhs);
        };
        ...
        GAPI_EXPORTS GKernelPackage combine(const GKernelPackage& lhs, const GKernelPackage& rhs);
         ...
         */
        template <typename... Ps> GKernelPackage combine(const GKernelPackage& a, const GKernelPackage& b, Ps&&... rest)
        {

        ...
        namespace wip
        {
            namespace draw
            {
                ...
                struct Text {
                     ...
                     * @param fs_                 The font scale factor that is multiplied by the font-specific base
                     * size
                     ...
                     * @param bottom_left_origin_ When true, the image data origin is at the bottom-left corner.
                     * Otherwise, it is at the top-left corner
                     */
                    Text(const std::string& text_, const cv::Point& org_, int ff_, double fs_, const cv::Scalar& color_,
                         int thick_ = 1, int lt_ = cv::LINE_8, bool bottom_left_origin_ = false)
                        : text(text_), org(org_), ff(ff_), fs(fs_), color(color_), thick(thick_), lt(lt_),
                          bottom_left_origin(bottom_left_origin_)
                    {
                    }
                    ...
                    bool bottom_left_origin; //!< When true, the image data origin is at the bottom-left corner.
                                             //!< Otherwise, it is at the top-left corner
                    /*@{*/
                };
                ...
                struct Rect {
                     ...
                     * @param thick_  The thickness of lines that make up the rectangle. Negative values, like #FILLED,
                     ...
                     */
                    Rect(const cv::Rect& rect_, const cv::Scalar& color_, int thick_ = 1, int lt_ = cv::LINE_8,
                         int shift_ = 0)
                ...
                struct Circle {
                     ...
                     * @param  thick_  The thickness of the circle outline, if positive. Negative values, like #FILLED,
                     * mean that a filled circle is to be drawn
                     * @param  lt_     The Type of the circle boundary. See #LineTypes
                     * @param  shift_  The Number of fractional bits in the coordinates of the center and in the radius
                     * value
                     */
                    Circle(const cv::Point& center_, int radius_, const cv::Scalar& color_, int thick_ = 1,
                           int lt_ = cv::LINE_8, int shift_ = 0)
                    ...
                    int shift; //!< The Number of fractional bits in the coordinates of the center and in the radius
                               //!< value
                    /*@{*/
                };
                ...
                struct Line {
                     ...
                     * @param  shift_  The number of fractional bits in the point coordinates
                     */
                    Line(const cv::Point& pt1_, const cv::Point& pt2_, const cv::Scalar& color_, int thick_ = 1,
                         int lt_ = cv::LINE_8, int shift_ = 0)
                ...
                struct Mosaic {
                     ...
                     */
                    Mosaic(const cv::Rect& mos_, int cellSz_, int decim_) : mos(mos_), cellSz(cellSz_), decim(decim_)
                    {
                    }
                ...
                struct Poly {
                     ...
                     */
                    Poly(const std::vector<cv::Point>& points_, const cv::Scalar& color_, int thick_ = 1,
                         int lt_ = cv::LINE_8, int shift_ = 0)
                ...
                */
                void GAPI_EXPORTS render(cv::Mat& bgr, const Prims& prims, cv::GCompileArgs&& args = {});
                ...
                @param args graph compile time parameters
                */
                void GAPI_EXPORTS render(cv::Mat& y_plane, cv::Mat& uv_plane, const Prims& prims,
                                         cv::GCompileArgs&& args = {});

                G_TYPED_KERNEL_M(GRenderNV12, <GMat2(cv::GMat, cv::GMat, cv::GArray<wip::draw::Prim>)>,
                                 "org.opencv.render.nv12"){
                    static GMatDesc2 outMeta(GMatDesc y_plane, GMatDesc uv_plane,
                                             GArrayDesc){return std::make_tuple(y_plane, uv_plane);
            } // namespace draw
        };    // namespace wip

        G_TYPED_KERNEL(GRenderBGR, <cv::GMat(cv::GMat, cv::GArray<wip::draw::Prim>)>,
                       "org.opencv.render.bgr"){static GMatDesc outMeta(GMatDesc bgr, GArrayDesc){return bgr;
    } // namespace gapi
};    // namespace cv
...
@param prims draw primitives
*/
GAPI_EXPORTS GMat2 renderNV12(const GMat& y, const GMat& uv, const GArray<Prim>& prims);
//! @} gapi_draw_api

...
namespace cv
{
    ...
    template <typename U> GArrayDesc descr_of(const std::vector<U>&)
    {
        return {};
    }
    static inline GArrayDesc empty_array_desc()
    {
        return {};
    }
    /** @} */
    ...
    namespace detail
    {
        ...
        template <typename T> struct TypeHint final : public TypeHintBase {
        };
        ...
        class GAPI_EXPORTS GArrayU
        {
          public:
            ...
            template <typename T> bool holds() const; // Check if was created from GArray<T>
          ...
          protected:
            ...
            template <typename T> void specifyType(); // Store type of initial GArray<T>

            std::shared_ptr<GOrigin> m_priv;
            std::shared_ptr<TypeHintBase> m_hint;
        };

        template <typename T> bool GArrayU::holds() const
        {
        ...
        template <typename T> void GArrayU::specifyType()
        {
        ...
        template <typename T> class VectorRefT final : public BasicVectorRef
        {
            ...
            {
                return util::holds_alternative<empty_t>(m_ref);
            }
            inline bool isROExt() const
            {
                return util::holds_alternative<ro_ext_t>(m_ref);
            }
            inline bool isRWExt() const
            {
                return util::holds_alternative<rw_ext_t>(m_ref);
            }
            inline bool isRWOwn() const
            {
                return util::holds_alternative<rw_own_t>(m_ref);
            }
          ...
          public:
            ...
            explicit VectorRefT(const std::vector<T>& vec) : m_ref(&vec)
            {
                init(&vec);
            }
            explicit VectorRefT(std::vector<T>& vec) : m_ref(&vec)
            {
                init(&vec);
            }
            explicit VectorRefT(std::vector<T>&& vec) : m_ref(std::move(vec))
            {
                init(&vec);
            }
            ...
            void reset()
            {
                if (isEmpty()) {
                    ...
                    GAPI_Assert(false); // shouldn't be called in *EXT modes
            }
            ...
            std::vector<T>& wref()
            {
                ...
                if (isRWExt())
                    return *util::get<rw_ext_t>(m_ref);
                if (isRWOwn())
                    return util::get<rw_own_t>(m_ref);
            ...
            const std::vector<T>& rref() const
            {
                ...
                //
                if (isROExt())
                    return *util::get<ro_ext_t>(m_ref);
                if (isRWExt())
                    return *util::get<rw_ext_t>(m_ref);
                if (isRWOwn())
                    return util::get<rw_own_t>(m_ref);
    ...
    #define FLATTEN_NS cv
        ...
        template <> struct flatten_g<cv::GMat> {
            using type = FLATTEN_NS::Mat;
        };
        template <> struct flatten_g<cv::GScalar> {

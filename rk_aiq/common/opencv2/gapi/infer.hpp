...
namespace cv
{
    ...
    namespace detail
    {
        ...
        template <typename, typename> struct KernelTypeMedium;
        ...
        template <class K, typename R, typename... Args>
        struct KernelTypeMedium<K, std::function<R(Args...)>> : public GKernelType<K, std::function<R(Args...)>> {
        };
    ...
    template <typename K, typename R, typename... Args> class GNetworkType<K, std::function<R(Args...)>>
    {
      public:
        using InArgs = std::tuple<Args...>;
        using OutArgs = std::tuple<R>;
    ...
    struct GInfer final : public GInferBase, public detail::KernelTypeMedium<GInfer<Net>, typename Net::API> {
    ...
    template <typename Net>
    struct GInferList final : public GInferListBase,
                              public detail::KernelTypeMedium<GInferList<Net>, typename Net::APIList> {
...
namespace cv
{
    namespace gapi
    {
         ...
         * @sa  G_API_NET()
         */
        template <typename Net, typename... Args> typename Net::ResultL infer(cv::GArray<cv::Rect> roi, Args&&... args)
        {
         ...
         * @sa  G_API_NET()
         */
        template <typename Net, typename... Args> typename Net::Result infer(Args&&... args)
        {
...
namespace cv
{
    ...
    namespace detail
    {
        template <typename T> gapi::GNetParam strip(T&& t)
        {
    ...
    namespace gapi
    {
        template <typename... Args> cv::gapi::GNetPackage networks(Args&&... args)
        {

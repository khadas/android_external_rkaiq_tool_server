...
namespace cv
{
    ...
    class GAPI_EXPORTS GFluidKernel
    {
      public:
        ...
        GFluidKernel(Kind k, int l, bool scratch, const F& f, const IS& is, const RS& rs, const B& b, const GW& win)
            : m_kind(k), m_lpi(l), m_scratch(scratch), m_f(f), m_is(is), m_rs(rs), m_b(b), m_gw(win)
        {
        }
    ...
    namespace detail
    {
        ...
        template <> struct CompileArgTag<GFluidParallelOutputRois> {
            static const char* tag()
            {
                return "gapi.fluid.parallelOutputRois";
            }
        };
    ...
    namespace detail
    {
        ...
        template <bool, typename Impl, typename... Ins> struct scratch_helper;

        template <typename Impl, typename... Ins> struct scratch_helper<true, Impl, Ins...> {
            // Init
            template <int... IIs>
            static void help_init_impl(const cv::GMetaArgs& metas, const cv::GArgs& in_args,
                                       gapi::fluid::Buffer& scratch_buf, detail::Seq<IIs...>)
            {
        ...
        template <typename Impl, typename... Ins> struct scratch_helper<false, Impl, Ins...> {
        ...
        template <bool CallCustomGetBorder, typename Impl, typename... Ins> struct get_border_helper;

        template <typename Impl, typename... Ins> struct get_border_helper<true, Impl, Ins...> {
            template <int... IIs>
            static gapi::fluid::BorderOpt get_border_impl(const GMetaArgs& metas, const cv::GArgs& in_args,
                                                          cv::detail::Seq<IIs...>)
            {
        ...
        template <typename Impl, typename... Ins> struct get_border_helper<false, Impl, Ins...> {
        ...
        template <bool CallCustomGetWindow, typename, typename... Ins> struct get_window_helper;

        template <typename Impl, typename... Ins> struct get_window_helper<true, Impl, Ins...> {
            template <int... IIs>
        ...
        template <typename Impl, typename... Ins> struct get_window_helper<false, Impl, Ins...> {
        ...
        template <typename C, typename T> struct has_Window {
          private:
            template <class U> static constexpr auto Check(U*) -> typename std::is_same<decltype(U::Window), T>::type;
        ...
        template <bool hasWindow, typename Impl> struct callCustomGetBorder;

        template <typename Impl> struct callCustomGetBorder<true, Impl> {
            static constexpr bool value = (Impl::Window != 1);
        };

        template <typename Impl> struct callCustomGetBorder<false, Impl> {
            static constexpr bool value = true;
        };

        template <typename, typename, typename, bool UseScratch> struct FluidCallHelper;
        ...
        struct FluidCallHelper<Impl, std::tuple<Ins...>, std::tuple<Outs...>, UseScratch> {
            ...
            static void call_impl(const cv::GArgs& in_args, const std::vector<gapi::fluid::Buffer*>& out_bufs,
            ...
            static void call(const cv::GArgs& in_args, const std::vector<gapi::fluid::Buffer*>& out_bufs)
            {
                ...
                call_impl(in_args, out_bufs, typename detail::MkSeq<sizeof...(Ins)>::type(),
            ...
            static gapi::fluid::BorderOpt getBorder(const GMetaArgs& metas, const cv::GArgs& in_args)
            {
                ...
                return get_border_helper<callCustomGetBorder<hasWindow, Impl>::value, Impl, Ins...>::help(metas,
                                                                                                          in_args);
            }
    ...
    template <class Impl, class K, bool UseScratch> class GFluidKernelImpl : public cv::detail::KernelTag
    {
      ...
      public:
        ...
        static GFluidKernel kernel()
        {
            ...
            return GFluidKernel(Impl::Kind, Impl::LPI, UseScratch, &P::call, &P::init_scratch, &P::reset_scratch,
                                &P::getBorder, &P::getWindow);
        }

        static cv::gapi::GBackend backend()
        {
            return cv::gapi::fluid::backend();
        }
    };

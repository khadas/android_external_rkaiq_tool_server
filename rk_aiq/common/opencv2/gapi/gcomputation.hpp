...
namespace cv
{
    ...
    namespace detail
    {
        ...
        template <typename...> struct last_type;

        template <typename T> struct last_type<T> {
            using type = T;
        };

        template <typename T, typename... Ts> struct last_type<T, Ts...> {
    ...
    class GAPI_EXPORTS GComputation
    {
      public:
        ...
        void apply(const std::vector<cv::gapi::own::Mat>& ins, // Compatibility overload
                   const std::vector<cv::gapi::own::Mat>& outs, GCompileArgs&& args = {});
        ...
        void apply(const std::vector<cv::Mat>& ins, // Compatibility overload
                   std::vector<cv::Mat>& outs, GCompileArgs&& args = {});
        ...
        auto compile(const Ts&... meta_and_compile_args) ->
            typename std::enable_if<detail::are_meta_descrs_but_last<Ts...>::value &&
                                    ...
                                    GCompiled>::type
        {
            // FIXME: wrapping meta_and_compile_args into a tuple to unwrap them inside a helper function is the
            // overkill

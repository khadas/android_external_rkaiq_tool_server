...
namespace cv
{
    ...
    struct GAPI_EXPORTS GMatDesc {
        ...
        GMatDesc(int d, int c, cv::gapi::own::Size s, bool p = false) : depth(d), chan(c), size(s), planar(p)
        {
        }

        GMatDesc(int d, const std::vector<int>& dd) : depth(d), chan(-1), size{-1, -1}, planar(false), dims(dd)
        {
        }

        GMatDesc(int d, std::vector<int>&& dd) : depth(d), chan(-1), size{-1, -1}, planar(false), dims(std::move(dd))
        {
        }
    ...
    static inline GMatDesc empty_gmat_desc()
    {
        return GMatDesc{-1, -1, {-1, -1}};
    }

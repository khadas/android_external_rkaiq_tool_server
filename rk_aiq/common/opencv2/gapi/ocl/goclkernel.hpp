...
namespace cv
{
    ...
    namespace detail
    {
        ...
        template <typename... Outputs> void postprocess_ocl(Outputs&... outs)
        {
            struct {
                void operator()(tracked_cv_umat* bm)
                {
                    bm->validate();
                }
                void operator()(scalar_wrapper_ocl* sw)
                {
                    sw->writeBack();
                }
                void operator()(...)
        ...
        template <typename, typename, typename> struct OCLCallHelper;
        ...
        struct OCLCallHelper<Impl, std::tuple<Ins...>, std::tuple<Outs...>> {
            template <typename... Inputs> struct call_and_postprocess {
                template <typename... Outputs> static void call(Inputs&&... ins, Outputs&&... outs)
                {
            ...
            static void call_impl(GOCLContext& ctx, detail::Seq<IIs...>, detail::Seq<OIs...>)
            {
                ...
                call_and_postprocess<decltype(ocl_get_in<Ins>::get(ctx, IIs))...>::call(
                    ocl_get_in<Ins>::get(ctx, IIs)..., ocl_get_out<Outs>::get(ctx, OIs)...);
            }
                           ...
                           public cv::detail::KernelTag
    {
      ...
      public:
        ...
        {
            return cv::gapi::ocl::backend();
        }
        static cv::GOCLKernel kernel()
        {
            return GOCLKernel(&P::call);
        }
    };

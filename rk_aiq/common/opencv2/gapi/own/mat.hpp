...
namespace cv
{
    namespace gapi
    {
        namespace own
        {
            namespace detail
            {
                template <typename T, unsigned char channels> void assign_row(void* ptr, int cols, Scalar const& s)
                {
                ...
                struct MatHeader {
                    ...
                    MatHeader(int _rows, int _cols, int type, void* _data, size_t _step)
                        : flags((type & TYPE_MASK)), rows(_rows), cols(_cols), data((uchar*)_data),
                          step(_step == AUTO_STEP ? detail::default_step(type, _cols) : _step)
                    {
                    }
            ...
            class Mat : public detail::MatHeader
            {
              public:
                ...
                */
                ...
                Mat(const std::vector<int>& _dims, int _type, void* _data) : MatHeader(_dims, _type, _data)
                {
                }

                Mat(std::vector<int>&& _dims, int _type, void* _data) : MatHeader(std::move(_dims), _type, _data)
                {
                }

                Mat(Mat const& src, const Rect& roi) : Mat(src)
                {
                    rows = roi.height;
                    cols = roi.width;
    ...
    {                                                                                                                  \
        assign_row<type, 1>, assign_row<type, 2>, assign_row<type, 3>, assign_row<type, 4>                             \
                    ...
                    static constexpr func_p_t func_tbl[][max_channels] = {
                        TABLE_ENTRY(uchar), TABLE_ENTRY(schar), TABLE_ENTRY(ushort), TABLE_ENTRY(short),
                        TABLE_ENTRY(int),   TABLE_ENTRY(float), TABLE_ENTRY(double)};
#undef TABLE_ENTRY

                    static_assert(CV_8U == 0 && CV_8S == 1 && CV_16U == 2 && CV_16S == 3 && CV_32S == 4 &&
                                      CV_32F == 5 && CV_64F == 6,
                ...
                int channels() const
                {
                    return dims.empty() ? CV_MAT_CN(flags) : -1;
                }
                ...
                */
                void create(Size _size, int _type)
                {
                    if (_size != Size{cols, rows}) {
                        ...
                        tmp.memory.reset(new uchar[tmp.step * tmp.rows], [](uchar* p) {
                            delete[] p;
                        });
                ...
                void create(const std::vector<int>& _dims, int _type)
                {
                    ...
                    tmp.memory.reset(new uchar[CV_ELEM_SIZE(_type) * sz], [](uchar* p) {
                        delete[] p;
                    });
                    tmp.data = tmp.memory.get();
                    *this = std::move(tmp);

...
namespace cv
{
    namespace gapi
    {
        namespace own
        {
            ...
            template <typename DST, typename SRC> static inline DST saturate(SRC x)
            {
                // only integral types please!
                GAPI_DbgAssert(std::is_integral<DST>::value && std::is_integral<SRC>::value);
                ...
                return x < std::numeric_limits<DST>::min()   ? std::numeric_limits<DST>::min()
                       : x > std::numeric_limits<DST>::max() ? std::numeric_limits<DST>::max()
                                                             : static_cast<DST>(x);
            }
            ...
            template <typename DST, typename SRC, typename R> static inline DST saturate(SRC x, R round)
            {
                if (std::is_floating_point<DST>::value) {
                    ...
                    GAPI_DbgAssert(std::is_integral<DST>::value && std::is_floating_point<SRC>::value);
#ifdef _WIN32

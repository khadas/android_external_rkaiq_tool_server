...
namespace cv
{
    ...
    namespace detail
    {
        ...
        struct tracked_cv_mat {
            ...
            void validate() const
            {
                if (r.data != original_data) {
                    util::throw_error(std::logic_error("OpenCV kernel output parameter was reallocated. \n"
        ...
        {
            struct {
                void operator()(tracked_cv_mat* bm)
                {
                    bm->validate();
                }
                void operator()(scalar_wrapper* sw)
                {
                    sw->writeBack();
                }
                void operator()(...)
        ...
        template <typename, typename, typename> struct OCVCallHelper;
        ...
        struct OCVCallHelper<Impl, std::tuple<Ins...>, std::tuple<Outs...>> {
            ...
            static void call_impl(GCPUContext& ctx, detail::Seq<IIs...>, detail::Seq<OIs...>)
            {
                ...
                call_and_postprocess<decltype(get_in<Ins>::get(ctx, IIs))...>::call(get_in<Ins>::get(ctx, IIs)...,

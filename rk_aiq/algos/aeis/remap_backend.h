...
namespace RkCam
{
    ...
    struct FecMeshBuffer final : public MeshBuffer {
        ...
        explicit FecMeshBuffer(const rk_aiq_fec_share_mem_info_t* mem)
            : FrameId(-1), ImageBufferIndex(-1), ImageBufferSize(0), Index(-1), Size(mem->size), Fd(mem->fd),
              UserPtr(mem->map_addr), MeshXi(mem->meshxi), MeshXf(mem->meshxf), MeshYi(mem->meshyi),
              MeshYf(mem->meshyf), State(mem->state){};

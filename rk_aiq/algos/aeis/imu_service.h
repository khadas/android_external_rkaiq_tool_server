...
namespace RkCam
{
    ...
    class EisImuData
    {
      public:
        EisImuData() = delete;
        explicit EisImuData(std::shared_ptr<EisImuAdaptor> imu, mems_sensor_type_t type, mems_sensor_event_t* data,
                            size_t count);
    ...
    class EisImuAdaptor : std::enable_shared_from_this<EisImuAdaptor>
    {
      public:
        ...
        explicit EisImuAdaptor(const rk_aiq_mems_sensor_intf_t intf, const mems_sensor_type_t type = SENSOR_ALL_TYPE);
        EisImuAdaptor(const EisImuAdaptor&) = delete;
        const EisImuAdaptor& operator=(const EisImuAdaptor&) = delete;
        ~EisImuAdaptor();

        constexpr const mems_sensor_type_t GetType()
        {
            return type_;
        };
        std::shared_ptr<EisImuAdaptor> GetPtr()
        {

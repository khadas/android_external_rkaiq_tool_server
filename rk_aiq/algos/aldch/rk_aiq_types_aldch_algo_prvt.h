...
#define MAP_TO_255LEVEL(level, level_max) ((float)(level) / 255 * (level_max));
...
class RKAiqAldchThread : public Thread
{
  public:
    RKAiqAldchThread(LDCHHandle_t ldchHandle) : Thread("ldchThread"), hLDCH(ldchHandle){};
    ~RKAiqAldchThread()
    {
        mAttrQueue.clear();
    };

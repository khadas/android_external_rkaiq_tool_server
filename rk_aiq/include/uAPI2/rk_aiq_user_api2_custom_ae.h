...
typedef struct rk_aiq_customeAe_cbs_s {
    ...
    int32_t (*pfn_ae_run)(void* ctx, const rk_aiq_customAe_stats_t* pstAeInfo, rk_aiq_customeAe_results_t* pstAeResult);
    /* not used now */
 ...
 */
XCamReturn rk_aiq_uapi2_customAE_register(const rk_aiq_sys_ctx_t* ctx, rk_aiq_customeAe_cbs_t* cbs);
 ...
 */
XCamReturn rk_aiq_uapi2_customAE_enable(const rk_aiq_sys_ctx_t* ctx, bool enable);
 ...
 */
XCamReturn rk_aiq_uapi2_customAE_unRegister(const rk_aiq_sys_ctx_t* ctx);

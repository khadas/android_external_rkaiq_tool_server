...
typedef struct AdegmmaParaV2_s {
    ...
    bool degamma_en;
    // M4_ARRAY_DESC("X_axis", "u16", M4_SIZE(1,17),  M4_RANGE(0, 4095),
    // "[0,256,512,768,1024,1280,1536,1792,2048,2304,2560,2816,3072,3328,3584,3840,4096]", M4_DIGIT(0), M4_DYNAMIC(0))
    int X_axis[ADEGAMMA_CRUVE_KNOTS];
    // M4_ARRAY_DESC("curve_R", "u16", M4_SIZE(1,17),  M4_RANGE(0, 4095),
    // "[0,256,512,768,1024,1280,1536,1792,2048,2304,2560,2816,3072,3328,3584,3840,4096]", M4_DIGIT(0), M4_DYNAMIC(0))
    int curve_R[ADEGAMMA_CRUVE_KNOTS];
    // M4_ARRAY_DESC("curve_G", "u16", M4_SIZE(1,17),  M4_RANGE(0, 4095),
    // "[0,256,512,768,1024,1280,1536,1792,2048,2304,2560,2816,3072,3328,3584,3840,4096]", M4_DIGIT(0), M4_DYNAMIC(0))
    int curve_G[ADEGAMMA_CRUVE_KNOTS];
    // M4_ARRAY_DESC("curve_B", "u16", M4_SIZE(1,17),  M4_RANGE(0, 4095),
    // "[0,256,512,768,1024,1280,1536,1792,2048,2304,2560,2816,3072,3328,3584,3840,4096]", M4_DIGIT(0), M4_DYNAMIC(0))

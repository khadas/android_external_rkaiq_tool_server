...
typedef struct CalibDbV2_Wb_Awb_Ext_Com_Para_t {
    ...
    // M4_ARRAY_DESC("lineRgBg", "f32", M4_SIZE(1,3), M4_RANGE(-2147483648,2147483647), "1", M4_DIGIT(4), M4_DYNAMIC(0),
    // M4_HIDE(1))
    float lineRgBg[3];
    // M4_ARRAY_DESC("lineRgProjCCT", "f32", M4_SIZE(1,3), M4_RANGE(-2147483648,2147483647), "1", M4_DIGIT(4),
...
typedef struct CalibDbV2_Wb_Awb_Para_V21_t {
    ...
    // M4_STRUCT_DESC("rgb2TcsPara", "normal_ui_style", M4_HIDE(1))
    CalibDbV2_Rgb2Tcs_t rgb2TcsPara;
    // M4_ARRAY_DESC("rgb2RotationYuvMat", "f32", M4_SIZE(4,4), M4_RANGE(-255,255), "0", M4_DIGIT(6), M4_DYNAMIC(0),
    // M4_HIDE(1))

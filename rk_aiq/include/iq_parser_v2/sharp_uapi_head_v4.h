...
typedef struct RK_SHARP_Params_V4_Select_s {
    ...
    // M4_ARRAY_DESC("luma_point", "s16", M4_SIZE(1,8), M4_RANGE(0,1024), "[0, 64, 128, 256, 384, 640, 896, 1024]",
    // M4_DIGIT(0), M4_DYNAMIC(0))
    int16_t luma_point[RK_SHARP_V4_LUMA_POINT_NUM];
    // M4_ARRAY_DESC("luma_sigma", "s16", M4_SIZE(1,8), M4_RANGE(0,1023), "[8, 10, 10, 12, 14, 12, 12, 10]",
    // M4_DIGIT(0), M4_DYNAMIC(0))
    ...
    // M4_ARRAY_DESC("hf_clip", "s16", M4_SIZE(1,8), M4_RANGE(0,1023), "[10, 20, 30, 56, 56, 56, 56, 30]", M4_DIGIT(0),
    // M4_DYNAMIC(0))
    int16_t hf_clip[RK_SHARP_V4_LUMA_POINT_NUM];
    // M4_ARRAY_DESC("local_sharp_strength", "s16", M4_SIZE(1,8), M4_RANGE(0,1023), "[1023, 1023, 1023, 1023, 1023,
    // 1023, 1023, 1023]", M4_DIGIT(0), M4_DYNAMIC(0))

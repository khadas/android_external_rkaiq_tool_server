...
#define j2s_root_struct_to_template_json(ctx) j2s_struct_to_template_json(ctx, NULL)
...
#define j2s_json_to_root_struct(ctx, json, ptr) j2s_json_to_struct(ctx, json, NULL, ptr)

/* Query cJSON from root struct */
#define j2s_json_from_root_struct(ctx, json, ptr) j2s_json_from_struct(ctx, json, NULL, ptr)

/* Store root struct to cache fd */
#define j2s_root_struct_to_cache(ctx, fd, ptr) j2s_struct_to_cache(ctx, NULL, fd, ptr)

/* Restore root struct from cache fd */
#define j2s_root_struct_from_cache(ctx, fd, ptr) j2s_struct_from_cache(ctx, NULL, fd, ptr)

/* Read file content to buf */
void* j2s_read_file(const char* file, size_t* size);

/* Apply JSON file to struct */
int j2s_json_file_to_struct(j2s_ctx* ctx, const char* file, const char* name, void* ptr);

/* Apply JSON file to root struct */
#define j2s_json_file_to_root_struct(ctx, file, ptr) j2s_json_file_to_struct(ctx, file, NULL, ptr)

char* j2s_dump_struct(j2s_ctx* ctx, const char* name, void* ptr);

/* Load/save root struct to cache file */
int j2s_load_struct_cache(j2s_ctx* ctx, const char* cache_file, void* ptr, void* auth_data, int auth_size);
void j2s_save_struct_cache(j2s_ctx* ctx, const char* cache_file, void* ptr, void* auth_data, int auth_size);
#define j2s_load_cache(ctx, cache_file, ptr) j2s_load_struct_cache(ctx, cache_file, ptr, NULL, 0)
#define j2s_save_cache(ctx, cache_file, ptr) j2s_save_struct_cache(ctx, cache_file, ptr, NULL, 0)
...
int j2s_modify_struct(j2s_ctx* ctx, const char* str, const char* name, void* ptr);

...
namespace RkCam
{
    ...
    typedef std::function<XCamReturn(std::vector<SmartPtr<XCamMessage>>&, uint32_t, uint64_t)> MessageHandleWrapper;
    ...
    class RkAiqAnalyzerGroup
    {
      public:
        ...
        RkAiqAnalyzerGroup(RkAiqCore* aiqCore, enum rk_aiq_core_analyze_type_e type, const uint64_t flag,
                           const RkAiqGrpConditions_t* grpConds, const bool singleThrd);
        virtual ~RkAiqAnalyzerGroup() = default;

        void setConcreteHandler(const MessageHandleWrapper handler)
        {
            mHandler = handler;
        }
        XCamReturn start();
        ...
        const rk_aiq_core_analyze_type_e getType() const
        {
            return mGroupType;
        }
        const uint64_t getDepsFlag() const
        {
            return mDepsFlag;
        }
        void setDepsFlag(uint64_t new_deps)
        {
            mDepsFlag = new_deps;
        }
    ...
    {
      public:
        RkAiqAnalyzeGroupMsgHdlThread(const std::string name, RkAiqAnalyzerGroup* group) : Thread(name.c_str())
        {
            if (group != nullptr)
                mHandlerGroups.push_back(group);
        };
        ~RkAiqAnalyzeGroupMsgHdlThread()
        {
            mMsgsQueue.clear();
        };
    ...
    class RkAiqAnalyzeGroupManager
    {
      ...
      protected:
        XCamReturn groupMessageHandler(std::vector<SmartPtr<XCamMessage>>& msgs, uint32_t id, uint64_t grpId);
        XCamReturn thumbnailsGroupMessageHandler(std::vector<SmartPtr<XCamMessage>>& msgs, uint32_t id, uint64_t grpId);

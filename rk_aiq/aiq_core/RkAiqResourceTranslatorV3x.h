...
namespace RkCam
{
    ...
    class RkAiqResourceTranslatorV3x : public RkAiqResourceTranslatorV21
    {
      public:
        struct Rectangle {
            uint32_t x;
        ...
        virtual XCamReturn translateAecStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqAecStatsProxy>& to);
        virtual XCamReturn translateAwbStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqAwbStatsProxy>& to);
        virtual XCamReturn translateAfStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqAfStatsProxy>& to);
        virtual XCamReturn translateAdehazeStats(const SmartPtr<VideoBuffer>& from,
                                                 SmartPtr<RkAiqAdehazeStatsProxy>& to);

        XCamReturn translateMultiIspStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqIspStatsIntProxy>& to)
        {
            return XCAM_RETURN_NO_ERROR;
        }
        XCamReturn translateMultiAecStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqAecStatsProxy>& to);
        XCamReturn translateMultiAwbStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqAwbStatsProxy>& to);
        XCamReturn translateMultiAfStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqAfStatsProxy>& to);
        XCamReturn translateMultiAdehazeStats(const SmartPtr<VideoBuffer>& from, SmartPtr<RkAiqAdehazeStatsProxy>& to);

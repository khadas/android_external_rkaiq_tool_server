...
#define grp_conds_array_info(x) x, grp_array_size(x)
    ...
    template <typename StandardMsg> class RkAiqCoreMsg : public XCamMessage
    {
      public:
        RkAiqCoreMsg(XCamMessageType msg_id, uint32_t id, const SmartPtr<StandardMsg>& message)
            : XCamMessage(msg_id, id), msg(message){};
    ...
    class RkAiqCoreThread : public Thread
    {
      public:
        RkAiqCoreThread(RkAiqCore* rkAiqCore) : Thread("RkAiqCoreThread"), mRkAiqCore(rkAiqCore){};
        ~RkAiqCoreThread()
        {
            mStatsQueue.clear();
        };
    ...
    class RkAiqCoreEvtsThread : public Thread
    {
      public:
        RkAiqCoreEvtsThread(RkAiqCore* rkAiqCore) : Thread("RkAiqCoreEvtsThread"), mRkAiqCore(rkAiqCore){};
        ~RkAiqCoreEvtsThread()
        {
            mEvtsQueue.clear();
        };
    ...
    class RkAiqCore : public rk_aiq_share_ptr_ops_t
    {
      ...
      public:
        ...
        XCamReturn calibTuning(const CamCalibDbV2Context_t* aiqCalib, ModuleNameList& change_list);
      ...
      protected:
        ...
        virtual void copyIspStats(SmartPtr<RkAiqAecStatsProxy>& aecStat, SmartPtr<RkAiqAwbStatsProxy>& awbStat,
                                  SmartPtr<RkAiqAfStatsProxy>& afStat, rk_aiq_isp_stats_t* to);
        virtual void setResultExpectedEffId(uint32_t& eff_id, enum RkAiqAlgoType_e type);
        void cacheIspStatsToList(SmartPtr<RkAiqAecStatsProxy>& aecStat, SmartPtr<RkAiqAwbStatsProxy>& awbStat,
      ...
      protected:
        ...
        XCamReturn handleIspStats(const SmartPtr<VideoBuffer>& buffer, const SmartPtr<RkAiqAecStatsProxy>& aecStat,

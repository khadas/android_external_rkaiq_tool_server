...
#define DEBUG 1
#ifndef TASK_LOG
    #define TASK_LOG(format, ...) printf("DEBUG: " #format "\n", ##__VA_ARGS__)
#endif
...
namespace XCam
{
    ...
    template <typename T, class Container = std::deque<ServiceParam<T>>> class TaskService
    {
      public:
        ...
        explicit TaskService(std::unique_ptr<ServiceTask<T>> task, const bool may_block_ = default_may_block,
                             const uint8_t max_param_count = default_max_param_count,
                             const TaskDuration max_process_time = default_process_time)
            : task_(std::move(task)), may_block_(default_may_block), max_param_count_(max_param_count),
              max_process_time_(max_process_time), started_(false)
        {
        ...
        void loop()
        {
            while (started_) {
                ...
                auto wait_ret = in_cond_.wait_for(in_lock, max_process_time_, [this]() {
                    return (!started_ ||
                            (!in_params_.empty() && in_params_.front().state == ParamState::kReadyForProcess));
                });
                if (wait_ret == false) {
                    // IDLE
                    continue;
                ...
                if (elapsed >= max_process_time_) {
                    ...
                    TASK_LOG("params processs elapsed %lf exceeds %lf", elapsed.count(), max_process_time_.count());
        ...
        ServiceParam<T> dequeue()
        {
            ...
            out_cond_.wait(lock, [this]() {
                if ((!out_params_.empty() && (out_params_.front().state == ParamState::kAllocated ||
        ...
        void flush()
        {
            ...
            auto paramIt = std::remove_if(in_params_.begin(), in_params_.end(), [](const ServiceParam<T>& p) {
                return (p.state != ParamState::kReadyForProcess) && (p.state != ParamState::kAllocated);
            });
        ...
        const uint8_t getMaxParamCount()
        {
            return max_param_count_;
        }
        void setMaxParamCount(const uint8_t count)
        {
            max_param_count_ = count;
        }

        const TaskDuration getMaxProceedTime()
        {
            return max_process_time_;
        }
        void setMaxProceedTime(const TaskDuration duration)
        {
            max_process_time_ = duration;
        }

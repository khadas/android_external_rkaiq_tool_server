...
namespace XCam
{
    ...
    struct VideoBufferPlanarInfo : XCamVideoBufferPlanarInfo {
        VideoBufferPlanarInfo();
    };

    struct VideoBufferInfo : XCamVideoBufferInfo {
        VideoBufferInfo();
        bool init(uint32_t format, uint32_t width, uint32_t height, uint32_t aligned_width = 0,
                  uint32_t aligned_height = 0, uint32_t size = 0, bool compacted = false);

        bool fill(const XCamVideoBufferInfo& info);

        bool get_planar_info(VideoBufferPlanarInfo& planar, const uint32_t index = 0) const;
    ...
    {
      public:
        explicit VideoBuffer(int64_t timestamp = InvalidTimestamp) : _timestamp(timestamp), _buf_type(0)
        {
        }
        explicit VideoBuffer(const VideoBufferInfo& info, int64_t timestamp = InvalidTimestamp)
            : _videoinfo(info), _timestamp(timestamp)
        {
        }
        virtual ~VideoBuffer();
        ...
        template <typename BufType> SmartPtr<BufType> find_typed_attach();

        bool add_metadata(const SmartPtr<MetaData>& data);
        bool remove_metadata(const SmartPtr<MetaData>& data);
        void clear_all_metadata();

        template <typename MetaType> SmartPtr<MetaType> find_typed_metadata();
        int _buf_type;
    ...
    template <typename BufType> SmartPtr<BufType> VideoBuffer::find_typed_attach()
    {
        for (VideoBufferList::iterator iter = _attached_bufs.begin(); iter != _attached_bufs.end(); ++iter) {
    ...
    template <typename MetaType> SmartPtr<MetaType> VideoBuffer::find_typed_metadata()
    {
        for (MetaDataList::iterator iter = _metadata_list.begin(); iter != _metadata_list.end(); ++iter) {

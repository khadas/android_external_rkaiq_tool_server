...
namespace XCam
{
    ...
    template <typename R, typename... Ts> struct Task {
        R operator()(Ts... args);
    };
    ...
    template <typename T> struct ServiceParam {
        ParamState state;           // params state
        int32_t unique_id;          // same as frame id
        std::shared_ptr<T> payload; // holds actual params
    };

    template <typename T> struct ServiceTask : public Task<TaskResult, ServiceParam<T>> {
        ServiceTask() = default;
        // Allow inherit class to dtor
        virtual ~ServiceTask() = default;
        virtual TaskResult operator()(ServiceParam<T>& p)
        {
            return TaskResult::kSkipped;
        }
    };

...
namespace XCam
{
    ...
    class DrmDevice
    {
      public:
        ...
        std::unique_ptr<DrmDumbObject> CreateDumbObject(unsigned int width, unsigned int height, unsigned int bpp = 8,
                                                        unsigned int num_planes = 1);
        XCamReturn DestroyDumbObject(const std::unique_ptr<DrmDumbObject>& bo);
        XCamReturn RequestMapDumbObject(const std::unique_ptr<DrmDumbObject>& bo, unsigned int plane = 0);

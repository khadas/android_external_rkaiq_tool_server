...
#define XCAM_STATIC_PROFILING_END(name, times_of_print)                                                                \
    ...
    if (name##_times >= times_of_print) {                                                                              \
        LOGI("profiling %s, fps:%.2f duration:%.2fms\n", #name, (name##_times * 1000.0f / name##_sum_time),            \
             name##_sum_time / name##_times);                                                                          \
        name##_times = 0;                                                                                              \
        name##_sum_time = 0.0;                                                                                         \
    }
    ...
    #define XCAM_OBJ_PROFILING_START gettimeofday(&_profiling_start_time, NULL)

    #define XCAM_OBJ_PROFILING_END(name, times)                                                                        \
        struct timeval profiling_now;                                                                                  \
        gettimeofday(&profiling_now, NULL);                                                                            \
        _profiling_sum_duration += (profiling_now.tv_sec - _profiling_start_time.tv_sec) * 1000.0f +                   \

...
namespace XCam
{
    ...
    template <class OBj> class SafeList
    {
      public:
    ...
    template <class OBj> typename SafeList<OBj>::ObjPtr SafeList<OBj>::pop(int32_t timeout)
    {
        SmartLock lock(_mutex);
        int code = 0;
    ...
    template <class OBj> bool SafeList<OBj>::push(const SafeList<OBj>::ObjPtr& obj)
    {
        SmartLock lock(_mutex);
    ...
    template <class OBj> bool SafeList<OBj>::erase(const SafeList<OBj>::ObjPtr& obj)
    {
        XCAM_ASSERT(obj.ptr());
        SmartLock lock(_mutex);
        for (SafeList<OBj>::ObjIter i_obj = _obj_list.begin(); i_obj != _obj_list.end(); ++i_obj) {
    ...
    template <class OBj> typename SafeList<OBj>::ObjPtr SafeList<OBj>::front()
    {
        SmartLock lock(_mutex);
    ...
    template <class OBj> void SafeList<OBj>::clear()
    {
        SmartLock lock(_mutex);

...
namespace XCam
{
    ...
    class RefObj
    {
      ...
      public:
        RefObj() : _ref_count(0)
        {
        } // derived class must set to SmartPtr at birth
        virtual ~RefObj()
        {
        }
    ...
    template <typename Obj> RefObj* generate_ref_count(Obj* obj, std::true_type)
    {
        XCAM_ASSERT(obj);
        obj->ref();
        return obj;
    ...
    template <typename Obj> RefCount* generate_ref_count(Obj*, std::false_type)
    {
        return new RefCount;
    }

    template <typename Obj> class SmartPtr
    {
      private:
      ...
      public:
        ...
        template <typename ObjDerive> SmartPtr(ObjDerive* obj) : _ptr(obj), _ref(NULL)
        {
            if (obj)
                init_ref(obj);
        }
        ...
        // copy from pointer
        SmartPtr(const SmartPtr<Obj>& obj) : _ptr(obj._ptr), _ref(obj._ref)
        {
            if (_ref) {
                _ref->ref();
                XCAM_ASSERT(_ptr);
        ...
        template <typename ObjDerive> SmartPtr(const SmartPtr<ObjDerive>& obj) : _ptr(obj._ptr), _ref(obj._ref)
        {
            if (_ref) {
                _ref->ref();
                XCAM_ASSERT(_ptr);
        ...
        template <typename ObjDerive> SmartPtr<Obj>& operator=(ObjDerive* obj)
        {
            release();
            set_pointer(obj, NULL);
            return *this;
        ...
        template <typename ObjDerive> SmartPtr<Obj>& operator=(const SmartPtr<ObjDerive>& obj)
        {
            release();
            set_pointer(obj._ptr, obj._ref);
            return *this;
        ...
        template <typename ObjDerive> SmartPtr<ObjDerive> dynamic_cast_ptr() const
        {
      ...
      private:
        template <typename ObjD> void set_pointer(ObjD* obj, RefObj* ref)
        {
            if (!obj)
                return;
        ...
        template <typename ObjD> void init_ref(ObjD* obj)
        {

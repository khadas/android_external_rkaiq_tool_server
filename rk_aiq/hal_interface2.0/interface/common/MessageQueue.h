...
NAMESPACE_DECLARATION
{
    ...
    template <class MessageType, class MessageId> class MessageQueue
    {

        // constructor / destructor
      public:
        MessageQueue(const char* name, int numReply)
            : mName(name), mNumReply(numReply), mReplyMutex(nullptr), mReplyCondition(nullptr), mReplyStatus(nullptr)
        {
            if (mNumReply > 0) {
      ...
      public:
        ...
        status_t receive(MessageType* msg, unsigned int timeout_ms = MESSAGE_QUEUE_RECEIVE_TIMEOUT_MSEC_INFINITE)
        {
            status_t status = NO_ERROR;

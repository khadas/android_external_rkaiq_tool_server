...
namespace XCam
{
    ...
    class RkAWBModeBase
    {
      public:
        ...
        virtual XCamReturn processState(const uint8_t& controlMode, const AwbControls& awbControls) = 0;

        virtual XCamReturn processResult(const rk_aiq_awb_results& awbResults, CameraMetadata& results) = 0;

        void resetState(void);
        uint8_t getState() const
        {
            return mCurrentAwbState;
        }
    ...
    class RkAWBModeAuto : public RkAWBModeBase
    {
      public:
        RkAWBModeAuto();
        virtual XCamReturn processState(const uint8_t& controlMode, const AwbControls& awbControls);
        virtual XCamReturn processResult(const rk_aiq_awb_results& awbResults, CameraMetadata& result);
    };
    ...
    class RkAWBStateMachine
    {
      public:
        ...
        virtual XCamReturn processResult(const rk_aiq_awb_results& awbResults, CameraMetadata& results);

        uint8_t getState() const
        {
            return mCurrentAwbMode->getState();
        }

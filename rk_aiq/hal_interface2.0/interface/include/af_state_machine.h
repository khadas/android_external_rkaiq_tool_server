...
namespace XCam
{
    ...
    class RkAfModeBase
    {
      public:
        ...
        virtual XCamReturn processTriggers(const uint8_t& afTrigger, const uint8_t& afMode, int preCaptureId,
                                           XCamAfParam& afInputParams);
        virtual XCamReturn processResult(rk_aiq_af_results& afResults, CameraMetadata& result) = 0;
        void resetState(void);
        void resetTrigger(usecs_t triggerTime);
        int getState()
        {
            return mCurrentAfState;
        }
    ...
    class RkAFModeAuto : public RkAfModeBase
    {
      public:
        ...
        virtual XCamReturn processTriggers(const uint8_t& afTrigger, const uint8_t& afMode, int preCaptureId,
                                           XCamAfParam& afInputParams);
        virtual XCamReturn processResult(rk_aiq_af_results& afResults, CameraMetadata& result);
    };
    ...
    class RkAFStateMachine
    {
      public:
        ...
        virtual XCamReturn processResult(rk_aiq_af_results& afResults, XCamAfParam& afInputParams,
                                         CameraMetadata& result);

        virtual XCamReturn updateDefaults(const rk_aiq_af_results& afResults, const XCamAfParam& afInputParams,
                                          CameraMetadata& result, bool fixedFocus = false) const;
        uint8_t getState() const
        {
            return mCurrentAfMode->getState();
        }
      ...
      private:
        ...
        void focusDistanceResult(const rk_aiq_af_results& afResults, const XCamAfParam& afInputParams,

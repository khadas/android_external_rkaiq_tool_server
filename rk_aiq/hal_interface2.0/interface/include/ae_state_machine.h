...
namespace XCam
{
    ...
    class RkAEModeBase
    {
      public:
        ...
        virtual XCamReturn processState(const uint8_t& controlMode, const AeControls& aeControls) = 0;

        virtual XCamReturn processResult(const rk_aiq_ae_results& aeResults, CameraMetadata& results,
                                         uint32_t reqId) = 0;

        void resetState(void);
        uint8_t getState() const
        {
            return mCurrentAeState;
        }
    ...
    class RkAEModeAuto : public RkAEModeBase
    {
      public:
        RkAEModeAuto();
        virtual XCamReturn processState(const uint8_t& controlMode, const AeControls& aeControls);
        virtual XCamReturn processResult(const rk_aiq_ae_results& aeResults, CameraMetadata& result, uint32_t reqId);
    };
    ...
    class RkAEStateMachine
    {
      public:
        ...
        virtual XCamReturn processResult(const rk_aiq_ae_results& aeResults, CameraMetadata& results, uint32_t reqId);

        uint8_t getState() const
        {
            return mCurrentAeMode->getState();
        }

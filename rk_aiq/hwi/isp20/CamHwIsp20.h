...
#define ISP_PARAMS_EFFECT_DELAY_CNT 2
    ...
    class CamHwIsp20 : public CamHwBase, virtual public Isp20Params, public V4l2Device, public isp_drv_share_mem_ops_t
    {
      public:
        ...
        virtual XCamReturn getSensorModeData(const char* sns_ent_name, rk_aiq_exposure_sensor_descriptor& sns_des);
      ...
      protected:
        ...
        void gen_full_isp_params(const struct isp2x_isp_params_cfg* update_params,
                                 struct isp2x_isp_params_cfg* full_params, uint64_t* module_en_update_partial,
                                 uint64_t* module_cfg_update_partial);
        void gen_full_ispp_params(const struct rkispp_params_cfg* update_params, struct rkispp_params_cfg* full_params);
        XCamReturn overrideExpRatioToAiqResults(const sint32_t frameId, int module_id, cam3aResultList& results,
                                                int hdr_mode);
#if 0
    void dump_isp_config(struct isp2x_isp_params_cfg* isp_params,
        ...
        XCamReturn setupPipelineFmtIsp(struct v4l2_subdev_selection& sns_sd_sel, struct v4l2_subdev_format& sns_sd_fmt,
                                       __u32 sns_v4l_pix_fmt);
        XCamReturn setupPipelineFmtCif(struct v4l2_subdev_selection& sns_sd_sel, struct v4l2_subdev_format& sns_sd_fmt,
                                       __u32 sns_v4l_pix_fmt);

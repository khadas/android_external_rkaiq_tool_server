...
namespace RkCam
{
    ...
    class RkStreamEventPollThread : public RkEventPollThread
    {
      public:
        explicit RkStreamEventPollThread(const char* thName, SmartPtr<V4l2Device> dev, CamHwIsp20* isp)
            : RkEventPollThread(thName, ISP_POLL_ISPSTREAMSYNC, dev, NULL), _pIsp(isp){};
    ...
    class RKStream
    {
      public:
        ...
        virtual SmartPtr<VideoBuffer> new_video_buffer(struct v4l2_event& event, SmartPtr<V4l2Device> dev)
        {
            return NULL;
        }
        virtual SmartPtr<V4l2BufferProxy> new_v4l2proxy_buffer(SmartPtr<V4l2Buffer> buf, SmartPtr<V4l2Device> dev)
        {
            return NULL;
        }
    ...
    {
      public:
        explicit SubVideoBuffer(int num, int index, int fd, const VideoBufferInfo& info) : VideoBuffer(info)
        {
            _buff_num = num;
            _buff_idx = index;
            _buff_fd = fd;
        ...
        explicit SubVideoBuffer(int fd) : VideoBuffer()
        {
            _buff_fd = fd;
            _buff_size = 0;
            _buff_ptr = MAP_FAILED;
        ...
        explicit SubVideoBuffer(SmartPtr<V4l2BufferProxy>& buf) : VideoBuffer()
        {
            _buff_fd = -1;
            _buff_size = 0;
            _buff_ptr = MAP_FAILED;
        ...
        explicit SubVideoBuffer() : VideoBuffer()
        {
            _buff_fd = -1;
            _buff_size = 0;
            _buff_ptr = MAP_FAILED;
        ...
        virtual uint8_t* map()
        {
            ...
            if (_buff_ptr == MAP_FAILED) {
                _buff_ptr =
                    mmap(NULL, _buff_size ? _buff_size : get_size(), PROT_READ | PROT_WRITE, MAP_SHARED, _buff_fd, 0);
                if (_buff_ptr == MAP_FAILED) {

...
#define CAPTURERAW_SUBM (0x80)
    ...
    class CaptureRawData
    {
      public:
        ...
        int dynamic_capture_raw(int i, uint32_t sequence, SmartPtr<V4l2BufferProxy> buf_proxy,
                                SmartPtr<V4l2Buffer>& v4l2buf, int mipi_dev_max, int working_mode,
                                SmartPtr<V4l2Device> dev);
        XCamReturn capture_raw_ctl(capture_raw_t type, int count = 0, const char* capture_dir = nullptr,
                                   char* output_dir = nullptr);
        XCamReturn notify_capture_raw();
        void save_metadata_and_register(uint32_t frameId, rkisp_effect_params_v20& ispParams,
                                        SmartPtr<RkAiqExpParamsProxy>& expParams, SmartPtr<RkAiqAfInfoProxy>& afParams,
                                        int working_mode);
      ...
      private:
        CaptureRawData();
        int calculate_stride_per_line(const struct capture_fmt& fmt, uint32_t& bytesPerLine, SmartPtr<V4l2Device> dev);
        const struct capture_fmt* find_fmt(const uint32_t pixelformat);
        XCamReturn creat_raw_dir(const char* path);
        XCamReturn write_frame_header_to_raw(FILE* fp, int dev_index, int sequence, int working_mode,
                                             SmartPtr<V4l2Device> dev);
        XCamReturn write_raw_to_file(FILE* fp, int dev_index, int sequence, void* userptr, int size);
        void write_reg_to_file(uint32_t base_addr, uint32_t offset_addr, int len, int sequence);
        void write_metadata_to_file(const char* dir_path, uint32_t frame_id, rkisp_effect_params_v20& ispParams,
                                    SmartPtr<RkAiqExpParamsProxy>& expParams, SmartPtr<RkAiqAfInfoProxy>& afParams,
                                    int working_mode);
